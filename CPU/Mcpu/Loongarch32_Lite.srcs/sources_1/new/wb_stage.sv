...
module wb_stage (
    ...
    output wire                   debug_wb_rf_wen,   // 供调试使用的寄存器写使能，上板测试时务必删除该信号
    output wire [`REG_ADDR_BUS  ] debug_wb_rf_wnum,  // 供调试使用的写寄存器地址，上板测试时务必删除该信号
    output wire [`WORD_BUS      ] debug_wb_rf_wdata  // 供调试使用的写寄存器数据，上板测试时务必删除该信号
);
